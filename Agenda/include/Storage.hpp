...
public:
  ...
  */
  std::list<Meeting>
  queryMeeting(std::function<bool(const Meeting &)> filter) const;
  ...
  */
  std::string findPassword(std::function<bool(const User &)> filter);
...
// A shared_ptr object effectively holds a pointer to the resource that it owns or holds a null pointer.
// A resource can be owned by more than one shared_ptr object; 
